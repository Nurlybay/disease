...
function show(user){const signed=!!user&&!user.is_anonymous;el('Login').hidden=signed;el('Register').hidden=signed;el('Logout').hidden=!signed;el('Name').hidden=!signed;el('Name').textContent=signed?(user.email||'Аккаунт'):'';}
...
form.addEventListener('submit',async e=>{e.preventDefault();if(busy||!form.reportValidity())return;busy=true;el('Submit').disabled=true;message('Подождите…');const submitted=mode;try{let result;
 ...
 else {show(result.data?.user||result.data?.session?.user);dialog.close();const status=document.getElementById('authStatus');status.hidden=false;status.textContent=submitted==='update'?'Пароль сохранён.':'Вы вошли в аккаунт.';}
