/* Загрузчик персонажа. Выбирает СЛУЧАЙНОГО из объединённого манифеста
   (встроенные manifest.js + кастомные и общие случаи
   CustomCases.manifestEntries()), подключает файл обычным <script>
   (работает и на file://, где fetch запрещён) и только после этого
   запускает app.js — приложение по-прежнему видит ровно один случай в
   window.CASES.

   Кастомный/общий случай не лежит файлом: черновик разворачивается через
   CustomCases.inflate(). Если подключён модуль синхронизации (общие
   случаи преподавателя из облака), выбор персонажа ждёт его готовности —
   без сети или без модуля старт мгновенный, как раньше.
...
(function () {
  ...
  function run() {
    ...
    function load(src, onDone) {
      ...
      s.src = src + '?v=patient-sounds-20260908';
      s.onload = onDone;
      s.onerror = function () {
  ...
  if (window.Sync) { Sync.init(run); } else { run(); }
})();
