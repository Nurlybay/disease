...
function remote(src){return /^characters\/(?!manifest\.js|loader\.js)[^/]+\.js$/.test(src)||['teacher.js','teacher-wizard.js','constructor.js','media-studio.js','clinical-media.js','media-lab.js'].includes(src);}
...
async function mediaRequest(body,endpoint='teacher-media'){
 const {data,error}=await auth.getSession();
 if(error||!data.session?.access_token)throw new Error('signin_required');
 const r=await fetch(config.url+'/functions/v1/'+endpoint,{method:'POST',headers:{apikey:config.anonKey,Authorization:'Bearer '+data.session.access_token,'Content-Type':'application/json'},body:JSON.stringify(body),signal:AbortSignal.timeout(120000)});
 const result=await r.json().catch(()=>({}));
 if(!r.ok)throw new Error(result.error||'service_unavailable');return result;
}
window.LearningAccess={loadScript,mediaRequest,caseRequest:body=>mediaRequest(body,'teacher-case'),isDemo:()=>demo&&!signed};
