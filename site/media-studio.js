...
(function () {
 ...
 '<div class="ms-body"><p>Студент спросит о жалобах или попросит показать нужную область — подходящая анимация запустится автоматически. Выбирать конкретный вопрос или осмотр не нужно.</p><button type="button" id="msSkip" class="btn btn-ghost">Продолжить без создания анимации</button>'+
 ...
 function media(job){return {description:job.description,image:job.image_url,video:job.video_url||'',detail:job.detail_image||'',patientKey:job.patient_key||'',synthetic:true,jobId:job.id};}
 function select(job){
