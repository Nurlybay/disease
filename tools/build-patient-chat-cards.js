...
box.window.CHARACTER_MANIFEST.forEach(function(entry){
  ...
  cards[c.id]={gender:c.patient.gender,statements:[{topic:'Жалоба при обращении',text:c.patient.greeting.text}].concat(c.passport,c.questions).map(function(s){return {topic:s.topic||s.label,text:s.text};})};
});
