...
"""
import asyncio, hashlib, json, pathlib, re, subprocess, tempfile
import edge_tts
...
async def main():
 for cid,(rate,pitch) in PROFILES.items():
  ...
  for text in texts:
   key=hashlib.sha256((rate+pitch+text).encode()).hexdigest()[:16]
   out=folder/(key+'.mp3')
   if not out.exists():
    for attempt in range(3):
     try:
      with tempfile.TemporaryDirectory(prefix='disease-tts-') as tmp:
       ...
       await asyncio.wait_for(edge_tts.Communicate(text,'ru-RU-DmitryNeural',rate=rate,pitch=pitch).save(str(raw)),45)
 ...
 print('All four patient voices generated and linked.',flush=True)
